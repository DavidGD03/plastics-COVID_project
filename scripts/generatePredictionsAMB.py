...
def plot_predicciones(testinverse,n_input,df_real_data):
    ...
    plt.legend(loc='best')
    plt.title("Real predictions using the "+modelo + " model and a window-size of "+str(n_input)+ " for " + region+'_'+escenario)
    plt.xlabel("Date")
    plt.ylabel("BMW Tons")
    plt.savefig(resultspath+'\\predictions_real_'+region+'_ws_'+ str(n_input)+"_"+modelo+'_'+escenario+"-model.png",dpi=fig.dpi)
...
def plot_training(generator,model,Y_scaler,n_input,train_MRNN_sc,df_real_data):
    ...
    plt.title("Train predictions using the "+modelo + " model and a window-size of "+str(n_input)+ " for " + region+'_'+escenario)
    ax.set_xlabel('Date')
    ax.set_ylabel("BMW Tons")
    ax.get_gid()
    ax.legend()
    plt.savefig(resultspath+'\\predictions_train_'+region+'_ws_'+ str(n_input)+"_"+modelo+'_'+escenario+"-model.png",dpi=fig.dpi)
...
def plot_test(test_MRNN_scN,model,Y_scaler,test_MRNN_sc,df_real_data):
    ...
    plt.title("Test predictions using the "+modelo + " model and a window-size of "+str(n_input)+ " for " + region+escenario)
    ax.set_xlabel('Date')
    ax.set_ylabel("BMW Tons")
    ax.get_gid()
    ax.legend()
    plt.savefig(resultspath+'\\predictions_test_'+region+'_ws_'+ str(n_input)+"_"+modelo+'_'+escenario+"-model.png",dpi=fig.dpi)
...
def plot_test2(testRNNM,model,Y_scaler,test_MRNN_sc,df_real_data):
    ...
    plt.plot(df_real_data[260:].index,df_real_data['BMW'][260:], label="Real data")
    plt.plot(testRNNM_predict.index,testRNNM_predict['Test'],label="Predictions")
    plt.title("Test predictions using the "+modelo + " model and a window-size of "+str(n_input) + " for " + region+'_'+escenario)
    ax.set_xlabel('Date')
    ax.set_ylabel("BMW Tons")
    ax.get_gid()
    ax.legend()
    plt.savefig(resultspath+'\\predictions_test2_'+region+'_ws_'+ str(n_input)+"_"+modelo+'_'+escenario+"-model.png",dpi=fig.dpi)
...
def main():
    ...
    plt.title("Number of epochs vs MSE using the "+modelo + " model and a window-size of "+str(n_input) + " for " + region+'_'+escenario)
    plt.xlabel("Epochs")
    plt.ylabel("Error")
    plt.legend()
    ...
    plt.savefig(resultspath+'\\epochs-mse_'+region+'_ws_'+ str(n_input)+"_"+modelo+'_'+escenario+"-model.png")
    ...
    errorMessage="Mean squared error using the "+modelo + " model and a window-size of "+str(window) + " for " + region+escenario + ': '+str(mse)
